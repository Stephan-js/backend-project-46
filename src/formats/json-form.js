...
const genDiff = (keys, files) => {
  ...
  const sett = {
    add: (rDirection, firstVal, val, result) => result.add.push({ name: firstVal, value: val, direction: `${rDirection}${firstVal}` }),
    ...
    delete: (rDirection, firstVal, result) => result.deleted.push({ name: firstVal, direction: `${rDirection}${firstVal}` }),
  };
